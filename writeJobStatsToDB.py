...
sql_to_create_master = "CREATE TABLE IF NOT EXISTS `master_table` (job_skill VARCHAR(255), type_of_skill VARCHAR(255), PRIMARY KEY(job_skill,type_of_skill))"
...
for each_files in glob.glob(general_folder_for_output + "/*" ):
    if ("lobal" not in each_files and "Run" not in each_files and "Error" not in each_files):
        file_string = each_files.replace("/all_urls_matching_keyword_","").replace(".txt","").replace(general_folder_for_output, "").strip()#.replace("(","").replace(")","").replace("^"," not ").replace("#"," Sharp").replace(" ","_").replace("-","_").strip()
        array_to_split_city = file_string.split("_",2)
        city_of_job = array_to_split_city[0]
        name_of_skill = array_to_split_city[1]
        all_types_of_skills = array_to_split_city[2]
        types_of_skills_array = all_types_of_skills.split("_")
        for each_type in types_of_skills_array:
            try:
                sql = "INSERT INTO `master_table` (`job_skill`, `type_of_skill`) VALUES (" + "'" + name_of_skill + "','" + each_type + "')"
                cursor.execute(sql)
                print "Ran SQL:" + sql + "\n"
            except:
